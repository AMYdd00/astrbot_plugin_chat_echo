...
### 新增
- ✨ **专属回复分析人设提示词**：支持为特定人格设置简化的专属回复分析提示词（`persona_replies`），大幅降低对话分析阶段的 Context / Token 占用，避免长人设爆 Token
- ✨ **专属定制设置页面 (Settings Page)**：WebUI 新增独立的插件管理与配置面板，支持分类定制基础与触发配置、回复与主动模式、关键词与图片配置、延迟批次判定、伪人设置及各人格回复分析人设的快速配置
- ✨ **系统级 API 接口支持**：新增一系列前端交互 Web API（获取人格列表、加载/保存专属人设提示词、获取/设置插件配置、获取可用 Provider 实例等）

### 优化
- 🔧 **项目结构重构与解耦**：将庞大的 `main.py` 和 `handlers.py` 进行彻底重构与模块化拆分，解耦为 `handlers/` 目录（批次处理、主动模式、回复模式、关键词触发独立逻辑）及 `services/` 目录（伪人模式、图片转述服务），极大地提升了项目的长期维护性
- 🔧 **唤醒屏蔽优化**：优化在回复监听状态（Active Tracking Window）下的原生 @ 唤醒和前缀唤醒屏蔽机制，避免在对话窗口期内由于用户 @ 触发而导致 Bot 生成双重/重复的 LLM 回复

## [1.1.3] - 2026-05-31

### 新增
- ✨ 延迟批次分析：自适应静默阈值 `clamp(avg_interval × 2.5, 3s, 12s)`，等群聊停顿后批量打包消息送给 LLM 分析，模拟真人聊天节奏
- ✨ 批次分析三重安全兜底：绝对超时(15s) + 最大消息数(6条) + @立即触发
- ✨ 图片转述后台预加载，批次触发时等待转述完成（可配置超时策略）
- ✨ 关键词规则支持**多关键词**（逗号分隔）和**按群设置生效范围**
- ✨ 群组配置支持**多群号**（逗号分隔），一套概率规则应用到多个群
- ✨ 所有逗号分隔字段兼容中英文逗号
- ✨ `keyword_rules` 和 `enabled_groups` 改造为 `template_list` 类型，WebUI 提供结构化模板编辑

### 优化
- 🔧 配置项按功能模块重新排序（基础→Provider→回复→主动→关键词→图片→批次→伪人）
- 🔧 UMO 格式全局支持（`enabled_groups`、`keyword_rules.groups` 均支持完整 UMO 和 suffix 匹配）

### 修复
- 🐛 修复 `/bot在干嘛` 和 `/bot计划表` 命令被 LLM 忽略的问题 — 改用 `@filter.command()` 标准命令拦截机制
- 🐛 修复未定义变量 `e` 导致批次分析异常日志丢失
- 🐛 修复重启后命令回复"暂无计划表" — 命令处理器主动调用 `_ensure_schedule` 懒加载日程
- 🐛 修复关键词路由与 reply 路由冲突导致 bot 双重相似回复 — 有活跃 reply tracker 时跳过关键词触发

## [1.1.2] - 2026-05-30

### 新增
- ✨ 新增**伪人模式**（`human_like_mode`），Bot 根据人格自主决定行为状态（如睡觉/忙碌/空闲），活跃度影响群聊参与概率
- ✨ 伪人模式支持 LLM 自动规划日程，日程用尽后自动刷新（重启/用尽触发，不再定时覆盖）
- ✨ 手动 @ 唤醒后不被日程刷新覆盖，下一个定时过渡点自然接管
- ✨ 伪人模式下被 @ 可唤醒：累计 @ 次数达标后强制唤醒 Bot 并恢复活跃
- ✨ 伪人模式加入随机打字延迟，模拟真人打字速度
- ✨ 新增指令 `/bot在干嘛`，群内任意时间可查询 Bot 当前状态和活跃度
- ✨ 新增指令 `/bot计划表`，群内可查询 Bot 完整的日程安排

### 优化
- 🔧 Token 统计页面移除「全部」按钮（与「近一年」时间范围等价）

## [1.1.1] - 2026-05-29

### 修复
- 🐛 修复 Token 统计页面切换"本周/本月"时折线图横坐标不随之变化的问题（现根据 period 动态传入 days 参数）
- 🐛 修复 `_get_period_range()` 缺少 `yesterday` 支持，导致昨日汇总数据错误

### 新增
- ✨ Token 统计页面新增「昨日」选项，可快速查看昨日用量汇总与折线图
- ✨ Token 统计折线图新增鼠标悬停 tooltip，显示每日各群 Prompt / Completion / Total 详细数据及竖线指示器
- ✨ 「今日」「昨日」「本周」折线图最低保证显示 7 天数据，避免单点无意义

### 优化
- 🔧 折线图切换时段新增 200ms 淡入动画
